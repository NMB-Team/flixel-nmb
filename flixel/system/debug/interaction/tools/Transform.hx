...
class Transform extends Tool {
	...
	final tooltip = new TransformTooltip();
	final markers:Array<Marker> = [];
	...
	override function activate():Void {
		...
		for (member in _brain.selectedItems)
			if (member != null && member.scrollFactor != null && member.isOnScreen()) {
				...
				for (marker in markers) marker.reposition(target);
	...
	private function checkMarkers():Void {
		...
		final closestDist = Marker.MOUSE_RADIUS;
		...
		if (closest != null) {
			...
			if (_brain.pointerJustPressed) startAction(closest.type);
		} else
			useDefaultCursor();
	}
	...
	override function update():Void {
		...
		switch state {
			case IDLE:
			case SELECTED(target): checkMarkers();
	...
	override function draw():Void {
		...
		final target = switch state {
			case IDLE: return;
			case SELECTED(target): target;
			case TRANSFORM(target, _, _, _): target;
		}
	...
	private function drawSelection(gfx:Graphics, camera:FlxCamera) {
		gfx.lineStyle(1, FlxColor.MAGENTA, 1, false, LineScaleMode.NORMAL, CapsStyle.SQUARE);

		// draw lines
		gfx.moveTo(markers[3].x, markers[3].y);
		for (marker in markers) gfx.lineTo(marker.x, marker.y);
...
private final class Marker {
	...
	public function reposition(target:FlxSprite) {
		final w = target.frameWidth * target.scale.x * .5;
		final h = target.frameHeight * target.scale.y * .5;
	...
	public static function draw(screenX:Float, screenY:Float, circle:Bool, gfx:Graphics) {
		...
		if (circle) gfx.drawCircle(screenX, screenY, CIRCLE_RADIUS);
		else gfx.drawRect(screenX - RECT_MARGIN, screenY - RECT_MARGIN, RECT_SIZE, RECT_SIZE);
		gfx.endFill();
	}
}

@:forward(setText) private abstract TransformTooltip(TooltipOverlay) to TooltipOverlay {
...
private enum abstract TransformType(String) to String {
	final ROTATE = "transformRotate";
	final SCALE_X = "transformScaleX";
	final SCALE_Y = "transformScaleY";
	final SCALE_XY = "transformScaleXY";
}
