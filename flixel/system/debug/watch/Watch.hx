...
class WatchBase<TEntry:WatchEntry> extends Window {
	#if FLX_DEBUG
	static inline final LINE_HEIGHT = 15;
	...
	public function add(displayName:String, data:WatchEntryData):Void {
		if (isInvalid(displayName, data)) return;

		final existing = getExistingEntry(displayName, data);
		if (existing != null) {
			switch (data) {
				case QUICK(_) | FUNCTION(_): existing.data = data;
				case _:
			}
			return;
		}
	...
	private function isInvalid(displayName:String, data:WatchEntryData):Bool {
		return switch (data) {
			...
			case EXPRESSION(expression, _): expression.isNullOrEmpty();
			case FUNCTION(func): func == null;
		}
	}
