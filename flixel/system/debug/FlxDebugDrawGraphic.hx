...
abstract FlxDebugDrawGraphic(Graphics) from Graphics to Graphics {
	...
	public function drawBoundingBox(x:Float, y:Float, width:Float, height:Float, color:FlxColor, thickness = 1.) {
		if (useHardware(width * height)) {
			...
			final half = thickness * .5;

			this.drawRect(x + half, y + half, width - thickness, height - thickness);
		}
	}

	public function drawLine(x1:Float, y1:Float, x2:Float, y2:Float, color:FlxColor, thickness = 1.) {
		this.lineStyle(thickness, color.rgb, color.alphaFloat);
