...
class CompletionList extends Sprite {
	...
	var selectedIndex = 0;
	var lowerVisibleIndex = 0;
	var upperVisibleIndex = 0;
	...
	private function createPopupEntries(amount:Int) {
		for (i in 0...amount) {
			final entry = new CompletionListEntry();
			entries.push(entry);
			addChild(entry);
	...
	public function onKeyDown(e:KeyboardEvent) {
		...
		switch (e.keyCode) {
			case Keyboard.DOWN: updateIndices(1);
			case Keyboard.UP: updateIndices(-1);
			case Keyboard.ENTER | Keyboard.TAB:
				if (completed != null) completed(items[selectedIndex]);
				close();
				return;
	...
	private function updateIndices(modifier:Int) {
		...
		if (FlxMath.inBounds(selectedIndex, lowerVisibleIndex, upperVisibleIndex)) return;

		// scroll visible area
		lowerVisibleIndex = bound(lowerVisibleIndex + modifier);
		upperVisibleIndex = bound(upperVisibleIndex + modifier);

		final range = upperVisibleIndex - lowerVisibleIndex;
		if (range == items.length) return;

		// hit lower or upper end
		if (lowerVisibleIndex == 0) upperVisibleIndex = entries.length - 1;
		else if (upperVisibleIndex == items.length - 1) lowerVisibleIndex = items.length - entries.length;
	}
	...
	private function updateLabels() {
		for (i in 0...entries.length) {
			...
			if (FlxStringUtil.isNullOrEmpty(selectedItem)) selectedItem = "";
			entries[i].setItem(selectedItem);
		}
	}

	private function updateSelectedItem() {
		for (entry in entries) entry.selected = false;
		entries[selectedIndex - lowerVisibleIndex].selected = true;

		if (selectionChanged != null) selectionChanged(items[selectedIndex]);
	}
	...
	private function setItems(items:Array<String>) {
		...
		selectedIndex = lowerVisibleIndex = 0;
		upperVisibleIndex = entries.length - 1;
		updateEntries();
	}

	private function filterItems(filter:String):Array<String> {
		if (FlxStringUtil.isNullOrEmpty(filter)) filter = "";

		return sortItems(filter, originalItems.filter(item -> return item.toLowerCase().contains(filter.toLowerCase())));
	}
	...
	private function sortItems(filter:String, items:Array<String>):Array<String> {
		if (FlxStringUtil.isNullOrEmpty(filter)) return items;

		items.sort((a, b) -> {
			final valueA = startsWithExt(a, filter);
			final valueB = startsWithExt(b, filter);

			if (valueA > valueB) return -valueA;
			if (valueB > valueA) return valueB;
			if (valueA == valueB) return Std.int(a.length - b.length);
	 ...
	 */
	inline function startsWithExt(s:String, start:String):Int {
		if (s.startsWith(start)) return 2;
		if (~/^[_]+/.replace(s, "").startsWith(start)) return 1;
		return 0;
	}

	@:noCompletion inline function set_filter(filter:String):String {
