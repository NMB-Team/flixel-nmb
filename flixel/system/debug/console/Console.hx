...
class Console extends Window {
	 ...
	 */
	static inline final DEFAULT_TEXT = #if hscript "(Click here / press [Tab] to enter command. Type 'help' for help.)" #else "Using the console requires hscript - please run 'haxelib install hscript'." #end;

	/**
	 * Map containing all registered Objects. You can use registerObject() or add them directly to this map.
	 */
	public var registeredObjects = new Map<String, Dynamic>();

	/**
	 * Map containing all registered Functions. You can use registerFunction() or add them directly to this map.
	 */
	public var registeredFunctions = new Map<String, Dynamic>();

	/**
	 * Map containing all registered help text. Set these values from registerObject() or registerFunction().
	 */
	public var registeredHelp = new Map<String, String>();
	...
	private function registerEventListeners() {
		...
		// openfl/openfl#1856
		FlxG.stage.addEventListener(KeyboardEvent.KEY_DOWN, (e:KeyboardEvent) -> if (FlxG.debugger.visible && FlxG.game.debugger.console.visible && e.keyCode == Keyboard.TAB) FlxG.stage.focus = input);
		#end

		#if sys // workaround for broken TextField focus on native
		input.addEventListener(MouseEvent.MOUSE_DOWN, Void -> inputMouseDown = true);
		FlxG.stage.addEventListener(MouseEvent.MOUSE_DOWN, Void -> stageMouseDown = true);
		#end
	}
	...
	override public function update() {
		...
		stageMouseDown = inputMouseDown = false;
	}
	#end
	...
	private function onKeyDown(e:KeyboardEvent) {
		...
		switch (e.keyCode) {
			case Keyboard.ENTER: if (input.text != "") processCommand();
			case Keyboard.ESCAPE: FlxG.stage.focus = null;
			case Keyboard.DELETE: input.text = "";
			case Keyboard.UP: if (!history.isEmpty) setText(history.getPreviousCommand());
			...
			#end
			case Keyboard.DOWN: if (!history.isEmpty) setText(history.getNextCommand());
		}
	}
	...
	private function processCommand() {
		try {
			...
			final output = ConsoleUtil.runCommand(text);
	...
	override public function destroy() {
		...
		input = FlxDestroyUtil.removeChild(this, input);

		registeredObjects = FlxArrayUtil.clear(registeredObjects);
		registeredFunctions = FlxArrayUtil.clear(registeredFunctions);
		registeredHelp = FlxArrayUtil.clear(registeredHelp);

		objectStack = FlxArrayUtil.clearArray(objectStack);
	}
