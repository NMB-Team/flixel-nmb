...
class SoundFrontEnd {
	 ...
	 */
	public var onVolumeChange(default, null) = new FlxTypedSignal<Float -> Void>();

	#if FLX_KEYBOARD
	public var keysAllowed = true;
	...
	public var countToChange(default, set) = 15;
	 ...
	 */
	public var soundTrayEnabled = true;
	 ...
	 */
	public var defaultMusicGroup = new FlxSoundGroup();

	/**
	 * The group sounds in load() / play() / stream() are added to unless specified otherwise.
	 */
	public var defaultSoundGroup = new FlxSoundGroup();

	/**
	 * A list of all the sounds being played in the game.
	 */
	public var list(default, null) = new FlxTypedGroup<FlxSound>();

	/**
	 * Set this to a number between 0 and 1 to change the global volume.
	 */
	public var volume(default, set) = 1.;
	 ...
	 */
	public function playMusic(embeddedMusic:FlxSoundAsset, volume = 1., looped = true, ?group:FlxSoundGroup):Void {
	 ...
	 */
	public function load(?embeddedSound:FlxSoundAsset, volume = 1., looped = false, ?group:FlxSoundGroup, autoDestroy = false, autoPlay = false, ?url:String, ?onComplete:Void -> Void, ?onLoad:Void -> Void):FlxSound {
	...
	private function loadHelper(sound:FlxSound, volume:Float, group:FlxSoundGroup, autoPlay = false):FlxSound {
	...
	public function play(embeddedSound:FlxSoundAsset, volume = 1.0, looped = false, ?group:FlxSoundGroup, autoDestroy = true, ?onComplete:Void -> Void):FlxSound {
		if ((embeddedSound is String)) embeddedSound = cache(embeddedSound);
		final sound = list.recycle(FlxSound).loadEmbedded(embeddedSound, looped, autoDestroy, onComplete);
	 ...
	 * @return  A FlxSound object.
	 */
	public function stream(url:String, volume = 1., looped = false, ?group:FlxSoundGroup, autoDestroy = true, ?onComplete:Void -> Void, ?onLoad:Void -> Void):FlxSound {
	 ...
	 */
	public function showSoundTray(up = false, ?forceSound = true):Void {
	...
	@:noCompletion final volumeMult = 1;
	...
	private function update(elapsed:Float):Void {
		...
		if (keysAllowed && !FlxInputText.globalManager.isTyping) {
			...
			if (justPressedMute) toggleMuted();
			else if (justPressedUp) changeVolume(volumeMult / countToChange);
			else if (justPressedDown) changeVolume(-volumeMult / countToChange);
	...
	private function onFocusLost():Void {
		music?.onFocusLost();
		for (sound in list.members) sound?.onFocusLost();
	}

	@:allow(flixel.FlxGame)
	private function onFocus():Void {
		music?.onFocus();
		for (sound in list.members) sound?.onFocus();
	}
	...
	private function loadSavedPrefs():Void {
		...
		if (FlxG.save.data.mute != null) muted = FlxG.save.data.mute;
	}
	#end
