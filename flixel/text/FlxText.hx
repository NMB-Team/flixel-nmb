...
class FlxText extends FlxSprite {
	 ...
	 */
	static inline final VERTICAL_GUTTER = 4;
	 ...
	 */
	public var borderColor(default, set) = FlxColor.TRANSPARENT;
	 ...
	 */
	public var borderQuality(default, set) = 1.;
	 ...
	 */
	var _graphicOffset = FlxPoint.get(0, 0);
	 ...
	 */
	public function new(x = .0, y = .0, fieldWidth = .0, ?text:String, size = 8, embeddedFont = true) {
		super(x, y);
	...
	public function stampOnAtlas(atlas:FlxAtlas):Bool {
		...
		final node = atlas.addNode(graphic.bitmap, graphic.key);
		final result = (node != null);

		if (node != null) frames = node.getImageFrame();
	 ...
	 *
	 * ```haxe
	 * final yellow = new FlxTextFormatMarkerPair(yellowFormat, "@");
	 * final green = new FlxTextFormatMarkerPair(greenFormat, "<g>");
	...
	public function applyMarkup(input:UnicodeString, rules:Array<FlxTextFormatMarkerPair>):FlxText {
		if (FlxStringUtil.isNullOrEmpty(rules)) return this; // there's no point in running the big loop

		clearFormats(); // start with default formatting

		final rangeStarts:Array<Int> = [];
		final rangeEnds:Array<Int> = [];
		final rulesToApply:Array<FlxTextFormatMarkerPair> = [];
		...
		for (rule in rules) {
			...
			var start = false;
			final markerLength = rule.marker.length;

			if (!input.contains(rule.marker)) continue; // marker not present

			// inspect each character
			for (charIndex in 0...input.length) {
				if ((input.substr(charIndex, markerLength):UnicodeString) != rule.marker) continue; // it's not one of the markers
			...
			if (start) rangeEnds.push(-1); // we ended with an unclosed block, mark it as infinite

			i++;
		}

		// Remove all of the markers in the string
		for (rule in rules) input = input.remove(rule.marker);
		...
		for (i in 0...rangeStarts.length) {
			...
			final markerLength = rulesToApply[i].marker.length;
			...
			delIndex = rangeEnds[i]; // Consider each range end
		...
		for (i in 0...rangeStarts.length) addFormat(rulesToApply[i].format, rangeStarts[i], rangeEnds[i]);
	 ...
	 */
	public function addFormat(format:FlxTextFormat, start = -1, end = -1):FlxText {
		...
		_formatRanges.sort((left, right) -> return left.range.start < right.range.start ? -1 : 1);
		_regen = true;
	...
	public function removeFormat(format:FlxTextFormat, ?start:Int, ?end:Int):FlxText {
		...
		while (i-- > 0) {
			final formatRange = _formatRanges[i];
			if (formatRange.format != format) continue;

			if (start != null && end != null) {
				final range = formatRange.range;
	 ...
	 */
	public function setFormat(?font:String, size:Int = 8, color = FlxColor.WHITE, ?alignment:FlxTextAlign, ?borderStyle:FlxTextBorderStyle, borderColor = FlxColor.TRANSPARENT, embeddedFont = true):FlxText {
		borderStyle = (borderStyle == null) ? NONE : borderStyle;

		if (embeddedFont) this.font = font;
		else if (font != null) systemFont = font;
	 ...
	 */
	public inline function setBorderStyle(style:FlxTextBorderStyle, color:FlxColor = 0, size = 1., quality = 1.):FlxText {
		borderStyle = style;
		borderColor = color;
		borderSize = size;
	...
	private function set_fieldWidth(value:Float):Float {
		if (textField == null) return value;
	...
	private function get_fieldHeight():Float {
		return (textField != null) ? textField.height : 0;
	}

	private function set_fieldHeight(value:Float):Float {
		if (textField == null) return value;
	...
	private function set_letterSpacing(letterSpacing:Float):Float {
		_defaultFormat.letterSpacing = letterSpacing;
		updateDefaultFormat();
		return letterSpacing;
	}

	override function setColorTransform(redMultiplier = 1., greenMultiplier = 1., blueMultiplier = 1., alphaMultiplier = 1., redOffset = .0, greenOffset = .0, blueOffset = .0, alphaOffset = .0) {
	...
	override function set_color(value:FlxColor):Int {
		if (_defaultFormat.color == value.rgb) return value;
	...
	private function set_font(font:String):String {
		...
		if (font != null) {
			...
			if (FlxG.assets.exists(font, FONT)) {
				final fontName = FlxG.assets.getFontUnsafe(font).fontName;
	...
	private function set_borderStyle(style:FlxTextBorderStyle):FlxTextBorderStyle {
		if (style != borderStyle) _regen = true;

		return borderStyle = style;
	}

	private function set_borderColor(color:FlxColor):FlxColor {
		if (borderColor != color && borderStyle != NONE) _regen = true;
	...
	private function set_borderQuality(value:Float):Float {
		...
		if (value != borderQuality && borderStyle != NONE) _regen = true;

		return borderQuality = value;
	}

	override function set_graphic(value:FlxGraphic):FlxGraphic {
		final oldGraphic = graphic;
		final graph:FlxGraphic = super.set_graphic(value);
	...
	private function regenGraphic():Void {
		...
		final oldWidth = graphic != null ? graphic.width : 0;
		final oldHeight = graphic != null ? graphic.height : VERTICAL_GUTTER;

		final newWidthFloat = textField.width;
		final newHeightFloat = _autoHeight ? textField.textHeight + VERTICAL_GUTTER : textField.height;
		...
		final newWidth = Math.ceil(newWidthFloat + borderWidth);
		final newHeight = Math.ceil(newHeightFloat + borderHeight);
		...
		if (textField.textHeight != 0 && (oldWidth != newWidth || oldHeight != newHeight)) {
			...
			final key = FlxG.bitmap.getUniqueKey("text");
	...
	override function calcFrame(runOnCpp = false):Void {
		if (textField == null || (FlxG.render.tile && !runOnCpp)) return;
	...
	inline function copyTextWithOffset(x:Float, y:Float) {
		final graphic:BitmapData = _hasBorderAlpha ? _borderPixels : graphic.bitmap;
		_matrix.translate(x, y);
		drawTextFieldTo(graphic);
	}

	private function applyFormats(formatAdjusted:TextFormat, useBorderColor = false):Void {
		...
		for (formatRange in _formatRanges) {
			if (textField.text.length - 1 >= formatRange.range.start) {
				final textFormat:TextFormat = formatRange.format.format;
				...
				break; // we can break safely because the array is ordered by the format start value
	...
	private function copyTextFormat(from:TextFormat, to:TextFormat, withAlign = true):Void {
		to.font = from.font;
		to.bold = from.bold;
	...
	private function dtfCopy():TextFormat {
		final dtf = textField.defaultTextFormat;
