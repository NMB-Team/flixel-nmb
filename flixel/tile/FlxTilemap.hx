...
class FlxTypedTilemap<Tile:FlxTile> extends FlxBaseTilemap<Tile> {
	 ...
	 */
	public var antialiasing(default, set) = FlxSprite.defaultAntialiasing;
	 ...
	 */
	public var tileWidth(default, null) = 0;

	/**
	 * The unscaled height of a single tile.
	 */
	public var tileHeight(default, null) = 0;

	/**
	 * The scaled width of a single tile.
	 */
	public var scaledTileWidth(default, null) = .0;

	/**
	 * The scaled height of a single tile.
	 */
	public var scaledTileHeight(default, null) = .0;
	...
	function new() {
		...
		if (FlxG.render.blit) FlxG.debugger.drawDebugChanged.add(onDrawDebugChanged);
		#end
	}
	...
	override function destroy():Void {
		...
		#if FLX_DEBUG
		if (FlxG.render.blit) FlxG.debugger.drawDebugChanged.remove(onDrawDebugChanged);
		#end
	...
	override function initTileObjects():Void {
		...
		var length = frames.numFrames;
		length += _startingIndex;

		for (i in 0...length) _tileObjects[i] = createTile(i, tileWidth, tileHeight);
	...
	private function createTile(index, width, height):Tile {
	...
	private function onCameraChanged(cam:FlxCamera):Void {
		_checkBufferChanges = true;
	}
	...
	override function cacheGraphics(tileWidth:Int, tileHeight:Int, tileGraphic:FlxTilemapGraphicAsset):Void {
		...
		final graph = FlxG.bitmap.add(cast tileGraphic);
		if (graph == null) return;

		// Figure out the size of the tiles
		if (tileWidth <= 0) tileWidth = graph.height;
		...
		if (actualFramePadding <= 0 || !graph.isLoaded) {
			...
			**/
			if (!graph.isLoaded && Std.isOfType(graph.bitmap, IEmbeddedBitmapData)) {
				final futureBitmap:IEmbeddedBitmapData = cast graph.bitmap;
				futureBitmap.onLoad = () -> frames = padTileFrames(tileWidth, tileHeight, graph, actualFramePadding);
	...
	private function padTileFrames(tileWidth:Int, tileHeight:Int, graphic:FlxGraphic, padding:Int) {
	...
	private function updateDebugTile(tileBitmap:BitmapData, color:FlxColor):BitmapData {
		if (FlxG.render.tile) return null;

		if (tileWidth <= 0 || tileHeight <= 0) return tileBitmap;
	...
	override function updateMap():Void {
		...
		final numTiles = _tileObjects.length;
		for (i in 0...numTiles)
			updateTile(i);
	}
	...
	override function drawDebugOnCamera(camera:FlxCamera):Void {
		...
		final l = FlxG.cameras.list.length;
		...
		var screenXInTiles = Math.floor(_point.x / scaledTileWidth);
		var screenYInTiles = Math.floor(_point.y / scaledTileHeight);
		final screenRows = buffer.rows;
		final screenColumns = buffer.columns;
		...
		var rowIndex = screenYInTiles * widthInTiles + screenXInTiles;
	...
	override function isOnScreen(?camera:FlxCamera):Bool {
		...
		final minX = x - offset.x - camera.scroll.x * scrollFactor.x;
		final minY = y - offset.y - camera.scroll.y * scrollFactor.y;
	...
	override function draw():Void {
		...
		for (i in 0...l) {
			...
			if (FlxG.render.blit) {
				if (buffer.isDirty(this, camera)) drawTilemap(buffer, camera);
	 ...
	 */
	override function setDirty(dirty = true):Void {
		if (FlxG.render.tile) return;

		for (buffer in _buffers)
			if (buffer != null) buffer.dirty = dirty;
	}
	...
	private function forEachOverlappingTileHelper(object:FlxObject, ?filter:(tile:Tile) -> Bool, ?position:FlxPoint, stopAtFirst:Bool):Bool {
		var xPos = x;
		var yPos = y;
		...
		final minTileX = bindInt(Math.floor((object.x - xPos) / scaledTileWidth), 0, widthInTiles);
		final minTileY = bindInt(Math.floor((object.y - yPos) / scaledTileHeight), 0, heightInTiles);
		final maxTileX = bindInt(Math.ceil((object.x + object.width - xPos) / scaledTileWidth), 0, widthInTiles);
		final maxTileY = bindInt(Math.ceil((object.y + object.height - yPos) / scaledTileHeight), 0, heightInTiles);
	...
	override function objectOverlapsTiles<TObj:FlxObject>(object:TObj, ?callback:(Tile, TObj) -> Bool, ?position:FlxPoint, isCollision = true):Bool {
		...
		function each(tile:Tile) {
			...
			if (overlapFound && callback != null) overlapFound = callback(tile, object);

			if (overlapFound) {
				if (tile.callbackFunction != null) tile.callbackFunction(tile, object);
	...
	override function getColumnAt(worldX:Float, bind = false):Int {
		...
		if (bind) return result < 0 ? 0 : (result >= widthInTiles ? widthInTiles - 1 : result);
	...
	override function getRowAt(worldY:Float, bind = false):Int {
		...
		if (bind) return result < 0 ? 0 : (result >= heightInTiles ? heightInTiles -1 : result);

		return result;
	}

	override function getColumnPos(column:Float, midpoint = false):Float {
		return x + column * scaledTileWidth + (midpoint ? scaledTileWidth * .5 : 0);
	}

	override function getRowPos(row:Int, midpoint = false):Float {
		return y + row * scaledTileHeight + (midpoint ? scaledTileHeight * .5 : 0);
	}
	...
	override function ray(start:FlxPoint, end:FlxPoint, ?result:FlxPoint):Bool {
		...
		if (start.x == end.x) {
			...
			if (hitIndex == -1) hitIndex = checkColumn(endTileX, lastTileY, endTileY);

			if (hitIndex != -1 && result != null) {
				result.copyFrom(getTilePos(hitIndex));
				if (Std.int(hitIndex / widthInTiles) == lastTileY) {
					if (start.x > end.x) result.x += scaledTileWidth;
					// set result to left side
					result.y = m * result.x + b;//mx + b
				} else {
					// if ascending
					if (start.y > end.y) result.y += scaledTileHeight; // change result to bottom
	...
	private function checkColumn(x:Int, startY:Int, endY:Int):Int {
		if (startY < 0) startY = 0;
		if (endY < 0) endY = 0;

		if (startY > heightInTiles - 1) startY = heightInTiles - 1;
		if (endY > heightInTiles - 1) endY = heightInTiles - 1;
		...
		while (true) {
			...
			if (tile != null && tile.solid) return index;
			if (y == endY) break;
	...
	public function tileToSprite(tileX:Int, tileY:Int, newTile = 0, ?spriteFactory:FlxTileProperties -> FlxSprite):FlxSprite {
		...
		if (tile != null && tile.visible) image = FlxImageFrame.fromFrame(tile.frame);
		else image = FlxImageFrame.fromEmptyFrame(graphic, FlxRect.get(0, 0, tileWidth, tileHeight));

		final worldX = tileX * tileWidth * scale.x + x;
		final worldY = tileY * tileHeight * scale.y + y;
		final tileSprite = spriteFactory({graphic: image, x: worldX, y: worldY, scale: FlxPoint.get().copyFrom(scale), alpha: alpha, blend: blend});

		if (newTile >= 0) setTileIndex(tileX, tileY, newTile);
	...
	private function drawTilemap(buffer:FlxTilemapBuffer, camera:FlxCamera):Void {
		...
		for (row in 0...screenRows) {
			...
			for (column in 0...screenColumns) {
				...
				if (tile != null && tile.visible && tile.frame.type != FlxFrameType.EMPTY) {
					...
					if (FlxG.render.blit) {
						...
						if (FlxG.debugger.drawDebug && !ignoreDrawDebug) {
							if (tile.allowCollisions == NONE) debugTile = _debugTileNotSolid;
							else if (tile.allowCollisions != ANY) debugTile = _debugTilePartial;
							else debugTile = _debugTileSolid;
						...
						if (frame.angle != FlxFrameAngle.ANGLE_0) frame.prepareMatrix(_matrix);
				...
				if (FlxG.render.blit) _flashPoint.x += tileWidth;

				columnIndex++;
			}

			if (FlxG.render.blit) _flashPoint.y += tileHeight;
			rowIndex += widthInTiles;
		}
		...
		if (FlxG.render.blit) {
			if (isColored) buffer.colorTransform(colorTransform);
			buffer.blend = blend;
		}
	...
	private function makeDebugTile(color:FlxColor):BitmapData {
		...
		final debugTile = new BitmapData(tileWidth, tileHeight, true, 0);
	...
	private function drawDebugTile(debugTile:BitmapData, color:FlxColor):Void {
		if (color != FlxColor.TRANSPARENT) {
			final gfx:Graphics = FlxSpriteUtil.flashGfx;
			gfx.clear();
			gfx.moveTo(0, 0);
	...
	override function updateTile(index:Int):Void {
		final tile:FlxTile = _tileObjects[index];
	...
	inline function createBuffer(camera:FlxCamera):FlxTilemapBuffer {
		final buffer = new FlxTilemapBuffer(tileWidth, tileHeight, widthInTiles, heightInTiles, camera, scale.x, scale.y);
	...
	private function set_antialiasing(value:Bool):Bool {
		for (buffer in _buffers) buffer.antialiasing = value;
	...
	private function set_color(value:FlxColor):Int {
	...
	private function defaultTileToSprite(tileProperties:FlxTileProperties):FlxSprite {
		final tileSprite = new FlxSprite(tileProperties.x, tileProperties.y);
...
typedef FlxTileProperties = {graphic:FlxImageFrame, x:Float, y:Float, scale:FlxPoint, alpha:Float, blend:BlendMode}
