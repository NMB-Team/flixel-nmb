...
@:multiType abstract FlxTypedSignal<T>(IFlxSignal<T>) {
	...
	@:to static inline function toSignal0(signal:IFlxSignal<Void -> Void>):FlxSignal0 {
		return new FlxSignal0();
	}

	@:to static inline function toSignal1<T1>(signal:IFlxSignal<T1 -> Void>):FlxSignal1<T1> {
		return new FlxSignal1();
	}

	@:to static inline function toSignal2<T1, T2>(signal:IFlxSignal<T1 -> T2 -> Void>):FlxSignal2<T1, T2>{
		return new FlxSignal2();
	}

	@:to static inline function toSignal3<T1, T2, T3>(signal:IFlxSignal<T1 -> T2 -> T3 -> Void>):FlxSignal3<T1, T2, T3> {
		return new FlxSignal3();
	}

	@:to static inline function toSignal4<T1, T2, T3, T4>(signal:IFlxSignal<T1 -> T2 -> T3 -> T4 -> Void>):FlxSignal4<T1, T2, T3, T4> {
...
private class FlxSignalHandler<T> implements IFlxDestroyable {
	...
	public var dispatchOnce(default, null) = false;
