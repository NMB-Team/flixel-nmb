...
class FlxSteamController {
	 ...
	 */
	public static var CONTROLLER_CONNECT_POLL_TIME(default, null) = .25;
	 ...
	 */
	public static var ORIGIN_DATA_POLL_TIME(default, null) = 1.;
	...
	static function getConnectedControllers():Array<Int> {
		...
		final arr = Steam.controllers.getConnectedControllers();
	...
	private static function getDigitalActionData(controller:Int, action:Int):ControllerDigitalActionData {
		...
		final data = Steam.controllers.getDigitalActionData(controller, action);
		if (controller >= 0 && controller < controllers.length)
...
@:allow(flixel.input.actions)
	...
	public var handle = -1;
	public var actionSet = -1;
	public var active = false;
	public var connected = new FlxInput<Int>(0);
