 ...
 */
@:keep class FlxGamepadAnalogStateList {
	...
	private function checkXY(id:FlxGamepadInputID):Bool {
		final stick = gamepad.mapping.getAnalogStick(id);
		...
		final xVal = checkRaw(stick.x, status);
		final yVal = checkRaw(stick.y, status);

		if (xVal && yVal) return true;

		if (xVal) {
			final yReleased = checkRaw(stick.y, RELEASED);
			final yJustReleased = checkRaw(stick.y, JUST_RELEASED);
			if (yReleased || yJustReleased) return true;
		}

		if (yVal) {
			final xReleased = checkRaw(stick.x, RELEASED);
			final xJustReleased = checkRaw(stick.x, JUST_RELEASED);
			if (xReleased || xJustReleased) return true;
		}
