...
abstract BMFontXml(Xml) from Xml {
	...
	inline function get_name() {
		return (this.nodeType == Xml.Document) ? "Document" : this.nodeName;
	}
	...
	public inline function new(xml:Xml) {
		if (xml.nodeType != Xml.Document && xml.nodeType != Xml.Element) throw 'Invalid nodeType ${xml.nodeType}';
		this = xml;
	}

	/** Check the existence of an attribute with the given name. **/
	public function has(name:String):Bool {
		if (this.nodeType == Xml.Document) throw 'Cannot access document attribute $name';
	...
	public function nodes(name:String):Array<BMFontXml> {
		return [for (xml in this.elementsNamed(name)) new BMFontXml(xml)];
	}
}
...
private abstract NodeAccess(Xml) from Xml to Xml {
	inline function getHelper(name:String, ?invalid:(String, ?PosInfos) -> Void):BMFontXml {
		...
		if (xml == null) {
			final xname = (this.nodeType == Xml.Document) ? "Document" : this.nodeName;
...
private abstract AttribAccess(Xml) from Xml to Xml {
	inline function stringHelper(name:String, ?invalid:(String, ?PosInfos) -> Void, ?backup:String):String {
		...
		if (this.nodeType == Xml.Document) {
			...
			if (v != null) value = v;
			else if (invalid != null) invalid('${this.nodeName} is missing attribute $name');
		}
		return value;
	}
	...
	inline function intHelper(name:String, ?invalid:(String, ?PosInfos) -> Void, backup:Int):Int {
		...
		if (this.nodeType == Xml.Document) {
			...
			final v = this.get(name);
			if (v != null) value = Std.parseInt(v);
			else if (invalid != null) invalid('${this.nodeName} is missing attribute $name');
		}
		return value;
	}
