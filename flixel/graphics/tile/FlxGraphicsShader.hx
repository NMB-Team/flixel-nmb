...
class FlxGraphicsShader extends GraphicsShader {
	...
	@:glFragmentHeader("
		...
		vec4 transform(vec4 color, vec4 mult, vec4 offset, float alpha) {
			color = clamp(offset + (color * mult), .0, 1.);
			return vec4(color.rgb, 1.) * color.a * alpha;
		}
		...
		vec4 applyFlixelEffects(vec4 color) {
			if (!hasTransform && !openfl_HasColorTransform) return color;

			color = mix(color, vec4(.0), float(color.a == .0));
		...
		vec4 flixel_texture2D(sampler2D bitmap, vec2 coord) {
			...
			if (isTexture) color.rgb *= color.a;
		...
		vec2 getCamPos(vec2 pos) {
			...
			return vec2(map(pos.x, size.x, size.x + size.z, 0.0, 1.), map(pos.y, size.y, size.y + size.w, .0, 1.));
		}

		vec2 camToOg(vec2 pos) {
			vec4 size = _camSize / vec4(openfl_TextureSize, openfl_TextureSize);
			return vec2(map(pos.x, 0.0, 1.0, size.x, size.x + size.z), map(pos.y, .0, 1., size.y, size.y + size.w));
		}
