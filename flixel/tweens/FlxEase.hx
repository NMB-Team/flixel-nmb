 ...
 * - **inOut(t)**
 *
 *       return (t <= .5) ? in(t * 2) * .5 : out(t * 2 - 1) * .5 + .5;
 */
class FlxEase {
	/** Easing constants */
	...
	static final ELASTIC_AMPLITUDE = 1.;
	static final ELASTIC_PERIOD = .4;
	...
	public static inline function smoothStepOut(t:Float):Float {
		return 2 * smoothStepInOut(t * .5 + .5) - 1;
	}
	...
	public static inline function smootherStepOut(t:Float):Float {
		return 2 * smootherStepInOut(t * .5 + .5) - 1;
	}
	...
	public static function bounceHardOut(t:Float):Float {
		...
		if (t < B2) return 1 - (7.5625 * (t -= B3) * t + .75);
		if (t < B4) return 1 - (7.5625 * (t -= B5) * t + .9375);
	...
	public static function backInOut(t:Float):Float {
		t *= 2;
		if (t < 1) return t * t * (2.70158 * t - 1.70158) * .5;
		t--;
	...
	public static function elasticInOut(t:Float):Float {
		if (t < .5) return -.5 * (Math.pow(2, 10 * (t -= .5)) * Math.sin((t - (ELASTIC_PERIOD * .25)) * (2 * Math.PI) / ELASTIC_PERIOD));
		return Math.pow(2, -10 * (t -= .5)) * Math.sin((t - (ELASTIC_PERIOD * .25)) * (2 * Math.PI) / ELASTIC_PERIOD) * .5 + 1;
	}
	...
	public static inline function powInOut(t:Float):Float {
		t *= 2;
		if (t < 1) return .5 * Math.pow(t, 3);
		return 1 - .5 * Math.abs(Math.pow(2 - t, 3));
	}
}
