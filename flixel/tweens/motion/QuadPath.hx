...
class QuadPath extends Motion {
	...
	override public function destroy():Void {
		super.destroy();
		// recycle FlxPoints
		for (point in _points) point = FlxDestroyUtil.put(point);
	 ...
	 */
	public function setMotion(durationOrSpeed:Float, useDuration = true):QuadPath {
		updatePath();
	 ...
	 */
	public function addPoint(x = .0, y = .0):QuadPath {
		_updateCurve = true;
	 ...
	 */
	public function getPoint(index = 0):FlxPoint {
		if (_points.length == 0) FlxG.log.critical("No points have been added to the path yet.");
	...
	private function updatePath():Void {
		if ((_points.length - 1) % 2 != 0 || _points.length < 3)
			FlxG.log.critical("A QuadPath must have at least 3 points to operate and number of points must be a odd.");
	...
	private function getCurveLength(start:FlxPoint, control:FlxPoint, finish:FlxPoint):Float {
		final safeControl = control.clone();
